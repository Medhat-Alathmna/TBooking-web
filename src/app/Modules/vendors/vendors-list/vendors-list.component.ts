...
export class VendorsListComponent extends BaseComponent implements OnInit {
  ...
  constructor(private calenderService: CalenderService) { super() }
  ...
  getVendorsList(pageNum?: number, query?: any) {
    ...
    this.loading = true
    const subscription = this.calenderService.getlist('vendors', pageNum, 10, query,'name').subscribe((results: any) => {
      this.loading = false
      if (!isSet(results)) {
        return
      }
