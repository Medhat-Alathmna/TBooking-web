...
export class PurchaseOrderFormComponent
  ...
  getVendorsList(pageNum?: number, query?: any) {
    ...
    const subscription = this.calenderService
      .getlist('vendors', pageNum, 10000, query, 'name')
      .subscribe(
        (results: any) => {
          this.loading = false;
          if (!isSet(results)) {
  ...
  getProducts(pageNum?: number, query?: any) {
    this.loading = true;
    const subscription = this.calenderService
      .getlist('products', pageNum, 1000, query)
      .subscribe(
        (results: any) => {
          this.loading = false;
          if (!isSet(results)) {
          ...
          results.data.map((item) => {
            this.products.push({
              ...
              brand: item?.attributes?.brand?.data?.attributes,
            });
          });
          subscription.unsubscribe();
        },
  ...
  updateStock() {
    const subscription = this.purchaseOrderService
      .updateStock(this.selectProducts)
      .subscribe(
        (data) => {
          if (!isSet(data)) {
            return;
  ...
  cashAllAmount() {
    ...
    let amountDedecte =
      this.getTotalPrice().productsAmount - this.getTotalPrice().totalPayments;
    this.payments.push({
      pay: amountDedecte,
      date: moment().format('YYYY-MM-DD hh:mm'),
      approved: false,
